...
/// an arrow between the two icons, and the two things worth knowing before first launch.
/// Drawn in code like the app icon (`AppIcon`); `build.sh dmg` renders it at 1× and 2× and
...
public enum DMGBackground {

    public static let width: CGFloat = 640
    public static let height: CGFloat = 440
    public static let iconSize: CGFloat = 128
    public static let appIcon = CGPoint(x: 170, y: 210)      // Finder coordinates, y down
    public static let appsIcon = CGPoint(x: 470, y: 210)
    ...
    public static func draw(signed: Bool) {
        let w = width, h = height
        paper.setFill()
        NSRect(x: 0, y: 0, width: w, height: h).fill()

        // Title and instruction.
        centered("Nimbus DMX Helper", font: .systemFont(ofSize: 22, weight: .semibold), color: ink, y: h - 52)
        ...
        let notes: [(String, String)] = [
            ("Hardware", "It drives an Enttec DMX USB Pro. Plug the widget in and press Connect \u{2014} "
                + "macOS\u{2019}s built-in FTDI driver handles it, there is nothing else to install."),
            ("One at a time", "Only one program can hold the widget\u{2019}s serial port, so quit anything else "
                + "talking to it first \u{2014} otherwise Connect will fail."),
        ]
    ...
    public static func pngData(scale: Int, signed: Bool) -> Data? {
    ...
    public static func write(to dir: String, signed: Bool) throws {
